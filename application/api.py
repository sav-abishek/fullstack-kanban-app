    ...
    try:
        if username == get_username_with_token():
            ...
            if user_we_want:
                ...
                raise NotFoundError(
                    404, "Username is incorrect or does not exist")
        else:
            raise AuthError(
                "Token is not valid for resource requeested", "UNAUTH")
    except:
        raise InternalError()
...
def card_object_if_deck_has_else_err(username, deck_id, card_id):
    ...
    if not found:
        raise ValidationError(
            404, f"{deck_we_want.name} have no such card", "NOCRD")
...
class Login(Resource):
    def post(self):
        ...
        if uname is None or pswd is None:
            raise ValidationError(
                status_code=400, error_message="Username or password can't be empty", error_code="EMPFLD")
        ...
        if user_we_want:
            if user_we_want.password == pswd:  # we can add some hasning things here later.
                ...
                raise ValidationError(
                    status_code=400, error_message="Incorrect Password", error_code="INCPW")
        else:
...
class Signup(Resource):
    def post(self):
        ...
        if args["username"] is None or args["name"] is None or args["email"] is None or args["pswd"] is None:
            raise ValidationError(
                status_code=400, error_message="one or more fields couldn't be empty", error_code="EMPFLD")
            # raise an error if fields are empty
        user_requested = User.query.filter(
            User.username == args["username"]).first()
        if user_requested:
            # if user is found
            raise ValidationError(
                status_code=409, error_message="Username exists", error_code="UNAEX")
        # try:
        new_user = User(username=args["username"], name=args["name"],
                        password=args["pswd"], email=args["email"], seen_today=True)
...
class Deck_details_with_username(Resource):
    ...
    def post(self, username):
        ...
        user_we_want = user_object_if_username_exists_else_err(username)
        try:
            new_deck = decks(name=args["name"], last_seen=dt.now().strftime(
                "%I:%M:%S %p %d-%b-%Y"))
        ...
        if user_we_want:
            ...
            for dk in user_we_want.decks:
                ...
                try:
                    ...
                    dk.score = 0
                dk.last_seen_human = timeago.format(dt.strptime(
                    dk.last_seen, '%I:%M:%S %p %d-%b-%Y'), dt.now())
    ...
    def post(self, username, deck_id):
        ...
        try:
            if request.headers.get('Content-Type') == 'application/json':
                ...
                args = request.form
            new_card = cards(
                question=args["question"], answer=args["answer"], score=0)
...
class individual_card(Resource):
    ...
    def get(self, username, deck_id, card_id):
        ...
        card_we_want = card_object_if_deck_has_else_err(
            username, deck_id, card_id)
        try:
...
api.add_resource(card_details_with_deck,
                 "/api/<string:username>/<int:deck_id>")
api.add_resource(
    individual_card, "/api/<string:username>/<int:deck_id>/<int:card_id>")
